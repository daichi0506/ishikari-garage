...
$Referer_check_domain = "ishikari-garage.com";
...
$require = array('お名前','電話番号','メールアドレス','ガレージタイプ','ご連絡方法');
...
<?php if($empty_flag == 1){ ?>
<div>
<h4>入力にエラーがあります。下記をご確認の上「前画面に戻る」ボタンにて修正をお願い致します。</h4>
<div style="color: #F94169;">
<?php echo $errm; ?><input type="button" value=" 前画面に戻る " onClick="history.back()">
</div>
<?php }else{ ?>
<h3 style="text-align: center;">こちらの内容でよろしいですか？</h3>
<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
<table class="formTable">
<?php echo confirmOutput($_POST);//入力内容を表示?>
</table>
<p style="display: inline-block;"><input type="hidden" name="mail_set" value="confirm_submit">
...
<div style="color:#F94169"><?php echo $errm; ?></div>
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div><h4>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4><div style="color:#F94169"><?php echo $errm; ?></div><br /><br /><input type="button" value=" 前画面に戻る " onClick="history.back()"></div>
<?php 
...
function refererCheck($Referer_check,$Referer_check_domain){
	if($Referer_check == 1 && !empty($Referer_check_domain)){
		if(strpos($_SERVER['HTTP_REFERER'],$Referer_check_domain) === false){
			return exit('<p>リファラチェックエラー。フォームページのドメインと、このファイルのドメインが一致しません</p>');
		}
	}
}
function copyright(){
